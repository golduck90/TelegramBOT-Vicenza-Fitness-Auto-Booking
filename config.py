...
if not TELEGRAM_BOT_TOKEN:
    raise RuntimeError(
        "❌ TELEGRAM_BOT_TOKEN non impostato. Mettilo in .env o exportalo."
    )

# ── WellTeam API ─────────────────────────────────────────
# AppToken: token company-level (uguale per tutti gli utenti della stessa palestra)
#          Ottienilo dall'app WellTeam o dal gestore della palestra
# AuthToken: personale, ottenuto dopo login
WELLTEAM_APP_TOKEN = os.environ.get("WELLTEAM_APP_TOKEN", "")
if not WELLTEAM_APP_TOKEN:
    raise RuntimeError(
        "❌ WELLTEAM_APP_TOKEN non impostato. Inseriscilo in .env."
    )
